...
public class TeleportEnemyController : MonoBehaviour
{
    ...
    [SerializeField] private NavMeshAgent mover = null;
    ...
    private void EvadePredator() 
    {
        ...
        mover.SetDestination(transform.position - (normDir * displacementDist));
    }
    #endregion
    ...
    private void Idle() 
    {
        if ((!mover.hasPath || mover.remainingDistance <= mover.stoppingDistance) && PassedTime(nextMoveTime))
        {
            nextMoveTime = SetFutureRandomTime(MIN_BTWN_MOVES, 5f);
            //mover.SetDestination(transform.position * 2);
        }
